...
public interface IMenuRepository extends CrudRepository<Menu, Integer> {
	...
	@Query(value ="(SELECT A.ID, A.TEXTO, A.NIVEL, A.RAIZ, A.URL FROM BVLSEGDB.MENU A INNER JOIN BVLSEGDB.MENU_ROL MR ON A.ID = MR.ID_MENU WHERE MR.ID_ROL = ?1)",nativeQuery = true)
